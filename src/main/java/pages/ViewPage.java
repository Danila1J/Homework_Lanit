...
public class ViewPage extends HelpdeskBasePage {
    ...
    public ViewPage checkTicket(Ticket ticket) {
        ...
        Assert.assertTrue(getDescription().contains(ticket.getDescription()),"Описание тикета не соотвествует");
        Assert.assertTrue(getQueue().contains(Dictionaries.getQueue(ticket.getQueue())),"Очередь тикета не соотвествует");
        Assert.assertTrue(getEmail().contains(ticket.getSubmitter_email()),"Email тикета не соотвествует");
        Assert.assertTrue(getPriority().contains(Dictionaries.getPriority(ticket.getPriority())),"Приоритет тикета не соотвествует");
        return this;
    }
    ...
    @Step("Получить очередь тикета")
    public String getQueue() {
        return queue.getText();
    }

    @Step("Получить Email тикета")
